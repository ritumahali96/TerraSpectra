...
var VERSION = true ? "9.3.11" : "latest";
...
var uniformBlock = `layout(std140) uniform densityUniforms {
  float factor;
} density;
`;
var densityUniforms = {
...
var VERSION2 = true ? "9.3.11" : "latest";
...
PostProcessTileLayer.layerName = "PostProcessTileLayer";
...
var VERSION3 = true ? "9.3.11" : "latest";
...
var VERSION4 = true ? "9.3.11" : "latest";
...
var FEATURE_BBOX_PROP = "_carto_bbox";
function parseFeatureBbox(value) {
...
function worldToTile(point, geoBbox, tileBbox) {
...
function createPointsFromPolygons(polygons, tileBbox, props, geoBbox) {
  const useBbox = Boolean(geoBbox && polygons.properties.length > 0 && FEATURE_BBOX_PROP in polygons.properties[0]);
    ...
    if (useBbox) {
      ...
      if (bbox) {
        ...
        if (bboxArea >= minPolygonArea) {
          const center = [(bboxWest + bboxEast) / 2, (bboxSouth + bboxNorth) / 2];
  ...
  renderSubLayers(props) {
    ...
    if (subLayers[0] && props.autoLabels) {
      ...
      if (props.data.polygons && props.data.polygons.positions.value.length > 0) {
        labelData.points = createPointsFromPolygons(props.data.polygons, this.state.mvt ? MVT_BBOX : tileBbox, props, tileBbox);
      }
